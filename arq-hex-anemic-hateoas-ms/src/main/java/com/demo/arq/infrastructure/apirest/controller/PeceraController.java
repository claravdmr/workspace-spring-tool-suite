...
public class PeceraController {
	...
	public ResponseEntity getPeceras(Pageable pageable) {
		...
		return ResponseEntity.ok(pagedResourcesAssembler.toModel(listDomain, peceraToPeceraDtoAssembler));
	}
	...
	public ResponseEntity getPecera(@Valid @PathVariable("pecera-id") String id) {
		...
		if (domain.isPresent()) {
			return ResponseEntity.ok(peceraToPeceraDtoAssembler.toModel(domain.get()));
		} else {
